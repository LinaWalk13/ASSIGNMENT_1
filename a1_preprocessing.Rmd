...
colnames(demo)[1:2] <- c("id", "visit")
colnames(lu)[1:2] <- c("id", "visit")
colnames(wd)[1:2] <- c("id", "visit")
...
demo <- select(demo, id, visit, Ethnicity, Diagnosis, Gender, Age, ADOS, MullenRaw, Socialization, ExpressiveLangRaw)

lu <- select(lu, id, visit, MOT_MLU, CHI_MLU)

wd <- select(wd, id, visit, types_MOT, types_CHI, tokens_MOT, tokens_CHI)
