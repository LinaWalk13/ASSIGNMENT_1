...
## Looking at the variables in the different datasets
names(demo)
names(lu)
names(wd)

## Renaming the variables "ID"
demo <- rename(demo, id = Child.ID)
lu <- rename(lu, id = SUBJ)
wd <- rename(wd, id = SUBJ)

## Renaming the variable "visit"
demo <- rename(demo, visit = Visit)
lu <- rename(lu, visit = VISIT)
wd <- rename(wd, visit = VISIT)
...
## Homogenizing visit and change the class to numeric
lu$visit <- as.numeric(str_extract(lu$visit, "\\d+"))
wd$visit <-as.numeric(str_extract(wd$visit, "\\d+"))
```
...
## Renaming and creating new dataset with id, visit and the 4 clinical variables
demo_new <- filter(demo, visit == 1) %>%
    select(id, 
           visit, 
           ADOS1 = ADOS, 
...
## Merging new data set with the old
joint_df <- merge(demo_new, joint_df, by = "id") ## i dont think this is correct hehehehe
```
